"""
Installable cryptographic primitives for VidProof.

Install once on any machine (including Raspberry Pi) with:
    pip install -e /path/to/vidproof

Both forensics/ and edge/ import from here so the crypto pipeline
is guaranteed to be identical in all modes.
"""
import base64
import hashlib
import secrets
...
from cryptography.hazmat.primitives.keywrap import InvalidUnwrap, aes_key_unwrap, aes_key_wrap  # noqa: F401
...
def encrypt_aes_gcm(plaintext: bytes) -> tuple[bytes, bytes, str, str]:
    ...
    The .enc file stores only ciphertext; the tag is stored separately in evidence.json.
    """
...
# ---------------------------------------------------------------------------
# X25519 key wrapping (ECDH + HKDF-SHA256 + AES-256-KW)
# ---------------------------------------------------------------------------
...
def wrap_aes_key(aes_key: bytes, owner_pubkey_b64: str) -> str:
    ...
    Encoded payload: ephemeral_pub_bytes[32] || wrapped_key_bytes[40] = 72 bytes → 96 char base64.
    """
    ...
    return base64.b64encode(ephemeral_pub_bytes + wrapped).decode()
...
def unwrap_aes_key(wrapped_key_b64: str, owner_privkey_pem_path: Path) -> bytes:
    ...
    Raises cryptography.hazmat.primitives.keywrap.InvalidUnwrap if tampered.
